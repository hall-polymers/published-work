...
hnbeads = nbeads / 2
nbeads_side = 12
nbeads_mid = hnbeads - nbeads_side * 2
nmonomersperpoly = 1
npoly = 480
hnpoly = npoly / 2
ntot = npoly * nbeads
...
sequence = [1] * hnbeads + [2] * hnbeads

# files
INPUT_LAMMPS = open("input_ring.lammps", "w")

# constants for grafting polymers
gnmonomers = nmonomersperpoly * npoly / nlayers
gntot = gnmonomers * hnbeads
gvol = (gntot) / dens
gnpoly = npoly / nlayers
surf = (gnpoly * 2) / 2 / cdens
ghx = surf ** 0.5
ghy = surf ** 0.5
ghz = (nbeads_side - 1) * bond + 1
ghx2 = ghx / 2.0
ghy2 = ghy / 2.0
ghz2 = ghz / 2.0
gvol = ghx * ghy * ghz
        ...
        for iy in range(nmonomersperpoly):
            ...
            for iz in seq:
                ...
                if iy == 0 and seqnum == 1:
                    ...
                    typeb[kk] = iz
                    molnum[kk] = ix + 1
                    if seqnum <= nbeads_side:
                        dz = bond
                        dx = 0
                    elif seqnum <= nbeads_side + nbeads_mid + 1:
                        dz = 0
                        dx = bond
                    else:
                        dz = -bond
...
# OUTPUT headers ---------------------------------------------------------------
INPUT_LAMMPS.write("# Kremer-Grest type Lamellae - Ring poly\n")
