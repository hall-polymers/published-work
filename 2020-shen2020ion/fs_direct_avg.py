...
fname = 'loglog.lammpstrj'
f = open(fname, 'r')
navg = 76  # averaging for this many different starting times
frameperblock = 38
timescale = 0.005
length_scale = [1, 2, 2.5, 3, 4, 5]
...
for length in length_scale:
	...
	fs1 = np.zeros((navg, frameperblock))
	fs1_ions = np.zeros((navg, frameperblock))
	# fs2 = np.zeros((navg, frames))
	for t in range(0, navg):
		print t
		t1 = t*frameperblock
		for t2 in range(t1, t1+frameperblock):
			dt = t2 - t1
			dr = np.array(r[t2][id2type==1])-np.array(r[t1][id2type==1])
			dr_ions = np.array(r[t2][(id2type==3)|(id2type==4)])-np.array(r[t1][(id2type==3)|(id2type==4)])
			
			fs1[t][dt] += sum (np.exp( -1.j * np.inner( [0,0,2*np.pi/length], dr))).real/num_type1/3
			fs1[t][dt] += sum (np.exp( -1.j * np.inner( [0,2*np.pi/length,0], dr))).real/num_type1/3
			fs1[t][dt] += sum (np.exp( -1.j * np.inner( [2*np.pi/length,0,0], dr))).real/num_type1/3

			fs1_ions[t][dt] += sum (np.exp( -1.j * np.inner( [0,0,2*np.pi/length], dr_ions))).real/num_ions/3
			fs1_ions[t][dt] += sum (np.exp( -1.j * np.inner( [0,2*np.pi/length,0], dr_ions))).real/num_ions/3
			fs1_ions[t][dt] += sum (np.exp( -1.j * np.inner( [2*np.pi/length,0,0], dr_ions))).real/num_ions/3
			#fs2[t][dt] += sum (np.exp( -1.j * np.inner( k_vector, np.array(r[t2][id2type==2])-np.array(r[t1][id2type==2])) ) ).real/num_type2

	file = 'fs_{}sigma.txt'.format(length)
	OUT = open(file, 'w')
	OUT.write("timesteps avg std\n")
	for dt in range(frameperblock):
		OUT.write("%7.3f " % (float(timestep[dt]-timestep[0])*timescale))
		OUT.write("%7f " % fs1.mean(axis=0)[dt])
		OUT.write("%7f\n" % fs1.std(axis=0)[dt])
		#OUT.write("%7f\n" % fs_final2[t])
	OUT.close()

	file = 'fs_{}sigma_ions.txt'.format(length)
	OUT = open(file, 'w')
	OUT.write("timesteps avg std\n")
	for dt in range(frameperblock):
		OUT.write("%7.3f " % (float(timestep[dt]-timestep[0])*timescale))
		OUT.write("%7f " % fs1_ions.mean(axis=0)[dt])
		OUT.write("%7f\n" % fs1_ions.std(axis=0)[dt])
		#OUT.write("%7f\n" % fs_final2[t])
	OUT.close()
