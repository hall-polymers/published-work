...
fname = 'PTMO2k_75pcent_log_dump.lammpstrj'
f = open(fname, 'r')
navg = 3  # averaging for this many different starting times
frameperblock = 42  # block size
nskip = 2  # skip this many frames between windows/blocks, added by NTL  
...
for length in length_scale:
	...
	for t in range(0, navg):  # different time origins
		...
		for t2 in range(t1, t1+frameperblock):
			dt = t2 - t1
			dr_ptmo = np.array(r[t2][(id2type==1)|(id2type==5)])-np.array(r[t1][(id2type==1)|(id2type==5)])  # type 1 and 5 are PTMO beads -- NTL
			...
			fs_ptmo[t][dt] += sum (np.exp( -1.j * np.inner( [0,0,2*np.pi/length], dr_ptmo))).real/num_ptmo_beads/3  # calculate sum( e^[ -i [ kz( riz(t2) - riz(t1) ) ] ] ) / 3N  :: NTL 
			fs_ptmo[t][dt] += sum (np.exp( -1.j * np.inner( [0,2*np.pi/length,0], dr_ptmo))).real/num_ptmo_beads/3  # calculate sum( e^[ -i [ ky( riy(t2) - riy(t1) ) ] ] ) / 3N  :: NTL
			fs_ptmo[t][dt] += sum (np.exp( -1.j * np.inner( [2*np.pi/length,0,0], dr_ptmo))).real/num_ptmo_beads/3  # calculate sum( e^[ -i [ kx( rix(t2) - rix(t1) ) ] ] ) / 3N  :: NTL
			...
			fs_hard[t][dt] += sum (np.exp( -1.j * np.inner( [2*np.pi/length,0,0], dr_hard))).real/num_hard_beads/3
                        ...
                        # Same process, but using different atom types (3 and 4) -- NTL
			...
			#fs2[t][dt] += sum (np.exp( -1.j * np.inner( k_vector, np.array(r[t2][id2type==2])-np.array(r[t1][id2type==2])) ) ).real/num_type2
	...
	file = 'fs_of_kt_{}sigma_ptmo_segment.dat'.format(length)
	OUT = open(file, 'w')
	OUT.write("timesteps avg std\n")
	for dt in range(frameperblock):  # iterate through all dt values -- NTL
		OUT.write("%7.3f " % (float(timestep[dt]-timestep[0])*timescale))  # output dt value in simulation units
		OUT.write("%7f " % fs_ptmo.mean(axis=0)[dt])  # average over all blocks/origins
		OUT.write("%7f\n" % fs_ptmo.std(axis=0)[dt])
		#OUT.write("%7f\n" % fs_final2[t])
	OUT.close()

	file = 'fs_of_kt_{}sigma_hard_segment.dat'.format(length)
	OUT = open(file, 'w')
	OUT.write("timesteps avg std\n")
