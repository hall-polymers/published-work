...
# %% Overview
# This Python 3 script generates a data file with the antiradial mean-squared
# internal distances and a data file with the average antiradial mean-squared
...
def get_command_arguments(arguments):
    if len(arguments) != 4:
        print("Unexpected number or arguments.\n"
              ...
              "<mean_squared_internal_distances_antiradial_data_file> "
              "<average_mean_squared_internal_distances_antiradial_data_file>"
              % argv[0])
        exit(1)
    return arguments
...
def main():
    ...
    mean_squared_internal_distances_antiradial_data_file = arguments[2]
    average_mean_squared_internal_distances_antiradial_data_file = arguments[3]
    frame = Frame()
    frame.read_unscaled_unwrapped_coordinates_frame_file(
        unscaled_unwrapped_coordinates_frame_file)
    frame.calculate_mean_squared_internal_distances_antiradial()
    frame.calculate_average_mean_squared_internal_distances_antiradial()
    frame.write_mean_squared_internal_distances_antiradial_data_file(
        mean_squared_internal_distances_antiradial_data_file)
    frame.write_average_mean_squared_internal_distances_antiradial_data_file(
        average_mean_squared_internal_distances_antiradial_data_file)
...
class Frame:
    def __init__(self):
        ...
        self.averageMeanSquaredInternalDistancesAntiRadial = []
        self.boxEdgeLength = []
    ...
    # Calculation of the Antiradial Mean-Squared Internal Distances of the
    # Grafted Chains in the Frame File with Unscaled and Unwrapped Coordinates
    def calculate_mean_squared_internal_distances_antiradial(self):
        index_final_atom = 1
        for index_atom in range(len(self.atoms)):
            initial_atom = self.atoms[index_atom]
            if initial_atom.type == 1:
                # The initial atom for the calculation of the antiradial
                # mean-squared internal distances of a grafted chain is always
                # set to be the last monomer of the grafted chain.
                index_final_atom = 1
                ...
                while final_atom.mol == initial_atom.mol:
                    ...
                    if index_atom + index_final_atom < len(self.atoms):
                        ...
                        break
                index_final_atom = index_atom + index_final_atom - 1
                final_atom = self.atoms[index_final_atom]
                index_initial_atom = 1
                initial_atom = \
                    self.atoms[index_final_atom - index_initial_atom]
                while initial_atom.mol == final_atom.mol:
                    dxu = initial_atom.xu - final_atom.xu
                    dyu = initial_atom.yu - final_atom.yu
                    dzu = initial_atom.zu - final_atom.zu
                    dru = sqrt(dxu ** 2 + dyu ** 2 + dzu ** 2)
                    dru2.append(dru ** 2)
                    index_initial_atom += 1
                    if index_final_atom - index_initial_atom >= 0:
                        initial_atom = \
                            self.atoms[index_final_atom - index_initial_atom]
                    else:
                        break
                index_final_atom = index_final_atom - index_atom + 1
                self.molecules.append(Molecule(initial_atom.mol, dru2))
        self.lengthGraftedChains = index_final_atom

    # Calculation of the Average Antiradial Mean-Squared Internal Distances for
    # the Grafted Chains in the Frame File with Unscaled and Unwrapped
    # Coordinates
    def calculate_average_mean_squared_internal_distances_antiradial(self):
        dru2_list = []
    ...
    # Export of the Data File with the Antiradial Mean-Squared Internal
    # Distances of the Grafted Chains in the Frame File with Unscaled and
    # Unwrapped Coordinates
    def write_mean_squared_internal_distances_antiradial_data_file(
            self,
            mean_squared_internal_distances_antiradial_data_file):
        file = open(
            mean_squared_internal_distances_antiradial_data_file, "w")
        file.write("Antiradial Mean-Squared Internal Distances "
                   "at Timestep %d\n"
                   % self.timestep)
        file.write("\n")
    ...
    # Export of the Data File with the Average Antiradial Mean-Squared Internal
    # Distances for the Grafted Chains in the Frame File with Unscaled and
    # Unwrapped Coordinates
    def write_average_mean_squared_internal_distances_antiradial_data_file(
            self,
            average_mean_squared_internal_distances_antiradial_data_file):
        file = open(
            average_mean_squared_internal_distances_antiradial_data_file, "w")
        file.write("Average Antiradial Mean-Squared Internal Distances "
                   "at Timestep %d\n"
                   % self.timestep)
        file.write("\n")
        for index_final_atom in range(self.lengthGraftedChains - 1):
