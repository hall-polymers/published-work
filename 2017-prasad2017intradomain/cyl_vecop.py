        ...
        for jj in range(2,nbeads):
            ...
            if typea[kk] == 2:
                ...
                for m in range(0,ncyl):
                    ...
                    if (ig < len(d1r)):
                        d2r[ig] = d2r[ig] + trr        #t_r
                        d2t[ig] = d2t[ig] + ttheta     #t_theta
                        d2z[ig] = d2z[ig] + zv         #t_z
                        nm1[ig] = nm1[ig] + 1.0
            else:
                xr = (xu[kk]-xu[kk-1])
                yr = (yu[kk]-yu[kk-1])
                ...
                for m in range(0,ncyl):
                    ig = int(sqrt(((xcmid[pp][kk] - xcom[m])-xbox*round((xcmid[pp][kk] - xcom[m])/xbox))**2+((ycmid[pp][kk] - ycom[m])-ybox*round((ycmid[pp][kk] - ycom[m])/ybox))**2)/drg) + 1
                    if ig < len(d1r):
                        d1r[ig] = d1r[ig] + trr        #t_r
                        d1t[ig] = d1t[ig] + ttheta    #t_theta
                        d1z[ig] = d1z[ig] + zv          #t_z
                        nm1[ig] = nm1[ig] + 1.0
...
for kconf in range(istart,nconf+iskip+1): 
    ...
    OUT = open(file, 'w')
    OUT.write("#%7i\n" % (conf))
    OUT.write("r t_r(A) t_theta(A) t_z(A) t_r(B) t_theta(B) t_z(B)\n")
    for ig in range(1,numg1):
