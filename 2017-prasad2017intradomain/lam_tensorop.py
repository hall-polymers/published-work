    ...
    for ii in range(0,npoly):
        for jj in range(2,nbeads):
            ...
            if jj < (nbeads/2+1):   #A monomer
                ...
                xt = ((xu[ii*nbeads+jj] - xu[ii*nbeads+jj-1])**2)/(xr+yr+zr) - float(1./3.)
                yt = ((yu[ii*nbeads+jj] - yu[ii*nbeads+jj-1])**2)/(xr+yr+zr) - float(1./3.)
                zt = ((zu[ii*nbeads+jj] - zu[ii*nbeads+jj-1])**2)/(xr+yr+zr) - float(1./3.)
                ...
                if zrc >= zcm:
                    ...
                    if ig < len(d1perp):
                        d1perp[ig] = d1perp[ig] + zt       #perp
                        d1par[ig] = d1par[ig] + xt + yt    #par
                        nm1[ig] = nm1[ig] + 1.0
                elif zrc < zcm:
                ...
                if zrc >= zcm:
                    ig = int((zrc - zcm) / drg) + 1
                    if ig < len(d1perp):
                        d2perp[ig] = d2perp[ig] + zt       #perp
                        d2par[ig] = d2par[ig] + xt + yt    #par
                        nm1[ig] = nm1[ig] + 1.0
                elif zrc < zcm:
                    ig = int((zrc+zbox - zcm) / drg) + 1
                    if ig < len(d1perp):
...
for kconf in range(istart,nconf+iskip+1): 
    ...
    OUT = open(file, 'w')
    OUT.write("#%7i\n" % (conf))
    OUT.write("z tenOP(A)_perp tenOP(B)_perp tenOP(A)_par tenOP(B)_par\n")
    for ig in range(1,numg1):
        OUT.write("%8.8f %8.8f %8.8f %8.8f %8.8f\n" % (ig,dd1perp[ig],dd2perp[ig],dd1par[ig],dd2par[ig]))
    OUT.close()
